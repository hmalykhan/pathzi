...
#     # If already expanded dict, still retrieve to get the latest fields (incl. confirmation_secret)
...
#     # ✅ NEW Stripe (Basil+): client secret is here
#     cs = (invoice.get("confirmation_secret") or {}).get("client_secret")
#     if cs:
#         return cs, hosted_url

#     # ✅ Legacy fallback: payment_intent (older Stripe responses)
#     pi = invoice.get("payment_intent")
...
def _get_invoice(subscription):
    """
    IMPORTANT:
    If latest_invoice is already expanded as a dict, DO NOT re-fetch it.
    Re-fetching without expand can drop confirmation_secret and cause missing client_secret.
    """
    inv = subscription.get("latest_invoice")
    if isinstance(inv, dict):
        return inv
...
def _get_client_secret_from_invoice(invoice):
    """
    Basil+ (new Stripe API): invoice.confirmation_secret.client_secret
    Legacy: invoice.payment_intent.client_secret
    """
    if not invoice:
        return None, None
...
def _get_setup_intent_client_secret(sub):
    """
    Sometimes Stripe returns a SetupIntent instead of a PaymentIntent for subscriptions.
    If present, PaymentSheet should use setupIntentClientSecret.
    """
    si = sub.get("pending_setup_intent")
    if isinstance(si, dict):
        return si.get("client_secret")
...
class SubscribeView(APIView):
    ...
    Returns:
      ...
      - ephemeral_key_secret
      - payment_intent_client_secret (when available)
      - setup_intent_client_secret (when available)
      - subscription_id
      - plan_id
    """
    permission_classes = [IsAuthenticated]
...
class ChangePlanView(APIView):
    ...
    def post(self, request):
        ...
        expand_fields = ["items", "latest_invoice.confirmation_secret", "pending_setup_intent", "schedule"]
